...
llvm::AllocaInst *Environment::createEntryBlockAlloca(std::string Identifer) {
  ...
  llvm::AllocaInst *Alloca = TmpB.CreateAlloca(
      llvm::Type::getDoubleTy(*CodeGen::TheContext), 0, Identifer.c_str());
  Locals[Identifer] = Alloca;
  return Alloca;
}

llvm::StoreInst *Environment::storeLocal(std::string Identifer,
                                         llvm::Value *Value) {
...
llvm::StoreInst *Environment::assignLocal(std::string Identifer,
                                          llvm::Value *Value) {
...
llvm::LoadInst *Environment::getLocal(std::string Identifer) {
  llvm::AllocaInst *Alloca = getAlloca(Identifer);
  if (Alloca) {
    return CodeGen::Builder->CreateLoad(
        llvm::Type::getDoubleTy(*CodeGen::TheContext), Alloca, Identifer);
  }
  return nullptr;
}

Environment::~Environment() { Locals.clear(); }
