...
llvm::Value* CallSmtAST::codegen() {
    ...
    if (!CalleeF)
        return CodeGen::error("Error calling function: Function not found the module");
...
llvm::Function* PrototypeAST::codegen() {
    ...
    llvm::FunctionType* FT = llvm::FunctionType::get(llvm::Type::getDoubleTy(*CodeGen::TheContext), Doubles, false);
...
llvm::Value* FunctionAST::codegen() {
    ...
    llvm::BasicBlock* EntryBlock = llvm::BasicBlock::Create(*CodeGen::TheContext, "entry", TheFunction);
    llvm::BasicBlock* ExitBlock = llvm::BasicBlock::Create(*CodeGen::TheContext, "exit");

    // Insert following IR into this block
    CodeGen::Builder->SetInsertPoint(EntryBlock);

    // Create a new environment
    CodeGen::CurrEnv = std::make_unique<Environment>(TheFunction, ExitBlock, std::move(CodeGen::CurrEnv));
...
llvm::Value* BinaryAST ::codegen() {
    ...
    switch (Op->Type) {
        ...
        case TOK_LESS:
            ...
            return CodeGen::Builder->CreateUIToFP(L, llvm::Type::getDoubleTy(*CodeGen::TheContext), "booltmp");
        case TOK_GREATER:
