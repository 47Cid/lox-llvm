...
class Visitor {
   public:
    ...
    virtual void visit(StringAST* e) = 0;
...
class StringAST : public SmtAST {
   public:
    std::string Value;
    StringAST(std::string Value) : Value(Value) {}
    ASTType getType() override { return ASTType::StringAST; }
...
class AssignAST : public SmtAST {
   public:
    ...
    AssignAST(Token* Name, TokenType Type, std::unique_ptr<SmtAST> Value, bool IsDecl)
        : Name(Name), Type(Type), Value(std::move(Value)), IsDecl(IsDecl) {}
    void accept(Visitor& v) override { v.visit(this); }
    llvm::Value* codegen() override;
};
...
class IfAST : public SmtAST {
   public:
    ...
    IfAST(std::unique_ptr<SmtAST> Cond,
          std::vector<std::unique_ptr<SmtAST>> Then,
