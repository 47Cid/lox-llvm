...
std::unique_ptr<llvm::Module> CodeGen::TheModule = std::make_unique<llvm::Module>("My Module", *TheContext);
std::unique_ptr<llvm::IRBuilder<>> CodeGen::Builder = std::make_unique<llvm::IRBuilder<>>(*TheContext);
std::map<std::string, llvm::Value*> CodeGen::NamedValues;
std::unique_ptr<Environment> CodeGen::CurrEnv = std::make_unique<Environment>(nullptr, nullptr, nullptr);
...
llvm::Value* CodeGen::insertExtFunctions() {
    std::vector<llvm::Type*> Doubles(1, llvm::Type::getDoubleTy(*CodeGen::TheContext));
