...
std::unique_ptr<SmtAST> Parser::error(std::string Message) {
    ...
    if (!PanicMode) std::cerr << Message << std::endl;
    PanicMode = true;
    return nullptr;
}
...
std::unique_ptr<SmtAST> Parser::call() {
    ...
    if (peek()->Type != TOK_LEFT_PAREN) return Expr;
...
std::unique_ptr<SmtAST> Parser::primary() {
    ...
    if (match(TOK_STRING)) {
        return std::make_unique<StringAST>(previous()->Lexeme);
    }
...
std::unique_ptr<SmtAST> Parser::comparison() {
    ...
    while (match(TOK_GREATER) || match(TOK_GREATER_EQUAL) || match(TOK_LESS) ||
           match(TOK_LESS_EQUAL)) {
        Token *Op = previous();
...
std::unique_ptr<SmtAST> Parser::assignment() {
    ...
    if (match(TOK_EQUAL)) {
        ...
        if (NewE == nullptr) {
            std::cerr << "Dynamic casting for variable failed";
            return nullptr;
        }
        return std::make_unique<AssignAST>(NewE->Name, TOK_TYPE_NUM, std::move(Value), false);
    }
    return E;
}
...
std::unique_ptr<AssignAST> Parser::varDecl() {
    ...
    consume(TOK_COLON, "Expected ':' after variable name");

    switch (peek()->Type) {
        case TOK_TYPE_NUM:
            advance();  // Consume the type
            consume(TOK_EQUAL, "Variable cannot be declared without a value");
            Value = expression();
            consume(TOK_SEMICOLON, "Expect ';' after variable declaration.");
            return std::make_unique<AssignAST>(Name, TOK_TYPE_NUM, std::move(Value), true);
            break;
        case TOK_TYPE_STRING:
            ...
            Value = expression();
            consume(TOK_SEMICOLON, "Expect ';' after variable declaration.");
            return std::make_unique<AssignAST>(Name, TOK_TYPE_STRING, std::move(Value), true);
            break;
        default:
